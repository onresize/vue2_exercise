<template>
  <div class="flex">
    <div class="item">
      <h2>景深效果</h2>
      <div class="wrapper">
        <div class="z1"></div>
        <div class="z2"></div>
      </div>
    </div>

    <div class="item">
      <h2>图片混合模式: background-blend-mode: lighten</h2>
      <div class="container"></div>
    </div>

    <div class="item">
      <h2>制作文字背景图: mix-blend-mode</h2>
      <div class="container1">
        <div class="pic"></div>
        <div class="text">IMAGE</div>
      </div>
    </div>

    <div class="item">
      <h2>background-blend-mode: 改变图标的颜色</h2>
      <div class="pic2 pic3"></div>
    </div>

    <div class="item">
      <h2>mix-blend-mode: 改变图标的颜色</h2>
      <div class="container2">
        <div class="pic4"></div>
      </div>
    </div>

    <div class="item">
      <div class="promo-card">
        <div class="promo-img-box"></div>
        <div class="inside-box">
          <p class="qq-number">QQ群：xxxxxxxx</p>
        </div>
      </div>
    </div>

    <div class="item">
      <div id="rocket" class="show"></div>
    </div>

    <div class="item">
      <div class="pic6"></div>
      <div class="pic6 pic7"></div>
      <div class="pic6 pic8"></div>
      <div class="pic6 pic9"></div>
    </div>
  </div>
</template>
 
<script>
export default {
  data() {
    return {}
  },
  computed: {},
  methods: {},
  watch: {},
  created() {},
  mounted() {},
}
</script>
 
<style scoped lang="less">
.flex {
  display: flex;
  justify-content: flex-start;
  flex-wrap: wrap;
  overflow: auto;
  height: 95vh;
  .item {
    margin: 10px;
  }
}

.wrapper {
  position: relative;
  z-index: 1;
  width: 300px;
  height: 300px;
  background: pink;
  transform-style: preserve-3d;
  perspective: 800px; // 人距离元素800px位置观看、增加空间感、https://www.cnblogs.com/yanggeng/p/11285856.html
  // transform: skew(-15deg, -15deg);

  .z1 {
    position: absolute;
    z-index: 2;
    width: 100px;
    height: 100px;
    background: #fff;
  }

  .z2 {
    position: absolute;
    z-index: 3;
    width: 100px;
    height: 100px;
    background: red;
  }
}

@pic1: 'https://user-images.githubusercontent.com/8554143/34369102-3ea353ac-eaf4-11e7-9d27-f7d28c6a66f6.jpg';
@pic2: 'https://user-images.githubusercontent.com/8554143/34369175-c14ae23e-eaf4-11e7-96f1-e146e5e5a96b.jpg';
.container {
  width: 400px;
  height: 500px;
  background: url(@pic1), url(@pic2);
  background-size: cover;
  background-blend-mode: lighten;
}

@img: 'https://picsum.photos/1000/1000?random=5';
.container1 {
  position: relative;
  width: 500px;
  height: 300px;
  margin: auto;
  .pic {
    position: relative;
    width: 100%;
    height: 100%;
    background: url(@img);
    background-repeat: no-repeat;
    background-size: 100% 100%;
  }

  .text {
    position: absolute;
    inset: 0;
    font-size: 140px;
    line-height: 295px;
    width: 100%;
    height: 100%;
    font-weight: bold;
    text-align: center;
    color: #000;
    mix-blend-mode: lighten;
    z-index: 99;
    background-color: #fff;
    border: 3px solid red;
  }
}

@img1: 'https://user-images.githubusercontent.com/8554143/34350345-4812d23a-ea51-11e7-98eb-461188ce9125.png';
.pic2 {
  width: 100px;
  height: 100px;
  margin: 50px auto;
  cursor: pointer;
  transition: 0.5s all ease-out;
}
.pic3 {
  background-image: url(@img1), linear-gradient(#f09, #09f, #f0f);
  background-blend-mode: lighten;
  background-size: cover;
  background-position: 0 0, 0 120px;
  background-repeat: no-repeat;
}
.pic3:hover {
  background-position: 0 0, 0 0;
}

.container2 {
  width: 100px;
  height: 100px;
  background: #000;
  transition: 0.5s background ease-out;
  transform: translate(70px, 70px);

  &:hover {
    background: #09f;
  }

  .pic4 {
    width: 101px;
    height: 101px;
    background-image: url(@img1);
    background-size: 100% 100%;
    cursor: pointer;
    background-position: 0 0;
    background-repeat: no-repeat;
    mix-blend-mode: lighten;
  }
}

.promo-card {
  position: relative;
  margin: 10px 0;
  width: 260px;
  height: 70px;
  border-radius: 5px;
  cursor: pointer;
  transition: all 0.7s;
  transition-timing-function: ease-out;
  transform-style: preserve-3d;

  > div {
    position: absolute;
    top: 0;
    right: 0;
    bottom: 0;
    left: 0;
    border-radius: 5px;
    box-shadow: 0 5px 5px -5px #000;
    margin-bottom: 10pt; // 1pt = 1.3px; pt为字体单位、意思是点
  }

  .promo-img-box {
    background: url('~@/assets/gif/o_read.gif') no-repeat center
      center;
    background-size: 100% 100%;
    transform: rotateX(0) translateZ(35px);
  }

  .inside-box {
    padding-top: 5px;
    background: -webkit-linear-gradient(top, #ebf0f3, #bfcbda);
    background: linear-gradient(180deg, #ebf0f3, #bfcbda);
    color: #333;
    text-indent: 20px;
    line-height: 26px;
    transform: rotateX(-90deg) translateZ(35px);
  }

  .qq-number {
    font-weight: 700;
    font-size: 22px;
  }

  .card-p {
    font-size: 9pt;
  }
  &:hover {
    transform: rotateX(90deg);
  }
}

#rocket {
  display: block;
  visibility: hidden;
  width: 26px;
  height: 3pc;
  background: url(data:image/png;base64,)
    no-repeat 50% 0;
  cursor: pointer;
  transition: all 0s;
}

#rocket:hover {
  background-position: 50% -62px;
}

#rocket.show {
  visibility: visible;
  opacity: 1;
}

#rocket.move {
  background-position: 50% -62px;
  animation: toTop 0.8s ease-in;
  animation-fill-mode: forwards;
}

@keyframes toTop {
  0% {
    opacity: 1;
    -webkit-filter: blur(0);
  }

  to {
    opacity: 0;
    transform: translate(0, -300px);
    -webkit-filter: blur(2px);
  }
}




@img4: 'https://user-images.githubusercontent.com/8554143/34238564-52f2ceb6-e63d-11e7-9812-dd987f98e9da.png';

.pic6 {
    width: 300px;
    height: 200px;
    margin: 30px;
    float: left;
    background: url(@img4),
        linear-gradient(#f00, #00f);
    background-size: cover, 100% 100%;
    background-position: 0 0, -300px 0;
    background-blend-mode: luminosity;
    background-repeat: no-repeat;
    transition: .5s background-position linear;
    cursor: pointer;
}

.pic7 {
    background-image: url(@img4),
        linear-gradient(90deg, #ff5722, #03a9f4); 
}

.pic8 {
    background-image: url(@img4),
        linear-gradient(-180deg, #00bcd4, #fa1a09); 
}

.pic9 {
    background-image: url(@img4),
        linear-gradient(45deg, #ffc107, #3f51b5, #4caf50, #ff0057, #fae105, #5c00ff); 
}

.pic6:hover { 
    background-position: 0 0, 0 0; 
}
</style>